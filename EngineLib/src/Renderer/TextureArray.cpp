...
namespace Engine
{
    ...
    void TextureArray::Destroy() { glDeleteTextures(1, &m_ID); }

    std::string_view TextureArray::GetName() const { return m_Spec.Name; }

    uint32_t TextureArray::ID() const { return m_ID; };

    uint32_t TextureArray::Width() const { return m_Spec.Width; }

    uint32_t TextureArray::Height() const { return m_Spec.Height; }

    uint32_t TextureArray::Channels() const { return m_Spec.Channels; }
}// namespace Engine