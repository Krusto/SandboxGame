...
namespace Engine
{
    DrawIndirectBuffer::DrawIndirectBuffer(uint8_t* data, size_t size, StorageBufferType type)
    {
        Init(data, size, type);
    }

    DrawIndirectBuffer DrawIndirectBuffer::Create(uint8_t* data, size_t size, StorageBufferType type)
    {
        return DrawIndirectBuffer(data, size, type);
    }

    void DrawIndirectBuffer::Init(uint8_t* data, size_t size, StorageBufferType type)
    {
    ...
    void DrawIndirectBuffer::Upload(uint8_t* data, size_t size, size_t offset) const
    {
        Renderer::GetInstance()->DrawIndirectBufferUpload(m_Data, (int8_t*) data, size, offset);
    }

    void DrawIndirectBuffer::Draw(size_t count, size_t stride, size_t offset) const
    {
