...
namespace Engine
{
    ...
    {
        m_Data = Engine::Allocator::AllocateArray<uint8_t>(CHUNK_SIZE_CUBIC_PADDED);
        for (size_t i = 0; i < CHUNK_SIZE_CUBIC_PADDED; i++) { m_Data[i] = 0; }
    }
    ...
    const std::size_t BlockData::Size() const { return CHUNK_SIZE_CUBIC; }
}// namespace Engine