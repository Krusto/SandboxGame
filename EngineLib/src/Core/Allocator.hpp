...
#define AllocatorIsLive(pptr, type)                                                                                    \
    ...
    public:
        ...
        template <typename T, std::size_t TypeSize = sizeof(T),
                  typename l = std::enable_if_t<!std::is_pointer<T>::value>, typename... Args>
    ...
    private:
        static void _DeallocateAll() noexcept
        {
            std::for_each(s_Allocations.begin(), s_Allocations.end(), [](auto& p) {
                std::size_t elementSize = p.second.size;
                std::size_t count = p.second.count;
        ...
        static constexpr void _Deallocate(T* ptr) noexcept
        {
            ...
            {
                std::size_t elementSize = s_Allocations[ptr].size;
                std::size_t count = s_Allocations[ptr].count;
                if (s_Allocations[ptr].destructor == nullptr) { s_AllocatedMemorySize -= elementSize * count; }
                else
                {
                    for (size_t i = 0; i < count; i++)
                    {
