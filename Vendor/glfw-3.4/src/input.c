...
void _glfwInputKey(_GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (key >= 0 && key <= GLFW_KEY_LAST)
    {
        ...
        if (action == GLFW_RELEASE && window->stickyKeys) window->keys[key] = _GLFW_STICK;
        else
...
void _glfwInitGamepadMappings(void)
{
    ...
    for (i = 0; i < count; i++)
    {
        if (parseMapping(&_glfw.mappings[_glfw.mappingCount], _glfwDefaultMappings[i])) _glfw.mappingCount++;
    }
}
